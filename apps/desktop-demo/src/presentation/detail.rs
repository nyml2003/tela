...
use tela_contract::{
    BorderRadius, Color, Fill, IconProvider, IdentityConcern, KeyStrategy, LayoutConcern,
    SemanticKey, Size, UiNode, VirtualListSpec, VisualConcern,
};
use tela_core::builder::LayoutContainer;
use tela_desktop_ui_kit::{Table, TableStyle, Td, Tr, VirtualWindow};
...
pub fn detail_pane(
    ...
    let body = match selected {
        ...
        Some(entry) if entry.kind != EntryKind::Folder => {
            asset_preview(entry, width, height - DETAIL_HEADER_H, icons)
        }
        _ => directory_detail(
            model,
            session,
    ...
    LayoutContainer::column([inline_summary(model, session, selected, width, icons), body])
...
fn inline_summary(
    ...
    .into();
    LayoutContainer::row([
        icon(kind_icon(entry.kind), kind_color(entry.kind), icons),
        name_stack,
        spacer(),
        text(
            if entry.favorite {
...
fn file_list(
    ...
    let rows: Vec<UiNode> = entries[window.range()]
        .iter()
        .map(|entry| file_row(entry, session.selected.contains(&entry.id), &columns, icons))
        .collect();
    Table::new(header)
    ...
    let rows: Vec<UiNode> = entries
        ...
        .map(|(row, group)| {
            let cards: Vec<UiNode> = group
                .iter()
                .map(|entry| thumbnail_card(entry, session.selected.contains(&entry.id), icons))
                .collect();
...
fn thumbnail_card(entry: &Entry, selected: bool, icons: &dyn IconProvider) -> UiNode {
...
fn asset_preview(entry: &Entry, width: f32, height: f32, icons: &dyn IconProvider) -> UiNode {
