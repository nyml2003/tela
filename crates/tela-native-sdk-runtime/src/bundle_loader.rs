...
mod tests {
    ...
    fn uses_network_once_then_validates_and_caches() {
        ...
        let index = serde_json::to_vec(&DevelopmentManifest {
            ...
            bundle_url: "/tela-dev/tela-demo.tela".to_owned(),
        ...
        let loaded = loader
            .load_with(
                ...
                |url| match url {
                    ...
                    "http://127.0.0.1:8000/tela-dev/tela-demo.tela" => Ok(archive.clone()),
    ...
    fn resolves_root_relative_bundle_against_remote_origin() {
        assert_eq!(
            ...
            Ok("http://host:8000/tela-dev/tela-demo.tela".to_owned())
        );
    }
}
