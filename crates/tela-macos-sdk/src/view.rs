...
impl TelaView {
    ...
    pub(crate) fn begin_close(&self) {
        ...
        if let Err(error) = state.pointer_left_client(self) {
            eprintln!("tela-macos-sdk: close pointer cleanup: {error}");
        }
        ...
        if let Err(error) = state.dispatch_text_channel_action(action) {
            eprintln!("tela-macos-sdk: close text channel: {error}");
        }
        state.gpu = None;
    }
...
impl ViewState {
    ...
    fn fail_startup(&mut self, view: &TelaView, error: String) {
        ...
        eprintln!("tela-macos-sdk: startup failed: {error}");
        self.runtime = None;
        self.frame = None;
        self.gpu = None;
    ...
    fn pointer(&mut self, view: &TelaView, event: AppEvent) -> Result<(), String> {
        if !self.lifecycle.can_render() {
            return Ok(());
        }
            ...
            match key_code {
                ...
                51 => {
                    let value = self
                        ...
                        .input_value
                        .clone();
                    self.dispatch_guest(AppEvent::SetInputValue(input::backspace(&value)))?;
                    self.request_redraw(view);
                    return Ok(());
                }
    ...
    fn receive_device_loss(&mut self, view: &TelaView) {
        ...
        match self.lifecycle.device_lost() {
            ...
            Some(DeviceLossAction::RecreateGpu) => {
                eprintln!(
                    "tela-macos-sdk: WGPU device lost, rebuilding once: {}",
                    report.detail
                );
                self.gpu = None;
