...
impl ControlledTextInput {
    ...
    pub(crate) fn publish(&mut self, status: MobileTextStatus) -> TextSync {
...
mod tests {
    ...
    fn blurred_input_rejects_platform_edits() {
        let mut input = ControlledTextInput::default();
        input.publish(MobileTextStatus::default());
