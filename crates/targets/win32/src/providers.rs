//! Win32 bridge providers: time, viewport, battery, and network (getCoordinates deferred).
//!
//! Platform calls go through the `windows` crate (safe wrappers); this module stays
//! unsafe-free. Viewport metrics are shared with the shell via `Rc<RefCell<...>>`.
