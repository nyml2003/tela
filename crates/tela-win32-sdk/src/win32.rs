...
pub fn run(options: PlatformLaunchOptions) -> Result<(), String> {
    ...
    if options.verbose {
        eprintln!(
            "tela-win32-sdk: startup index={} cache={}",
...
impl WindowState {
    ...
    fn fail_startup(&mut self, error: String) {
        ...
        eprintln!("tela-win32-sdk: startup failed: {error}");
        self.runtime = None;
        self.frame = None;
        self.gpu = None;
    ...
    fn receive_device_loss(&mut self) {
        ...
        match self.lifecycle.device_lost() {
            ...
            Some(DeviceLossAction::RecreateGpu) => {
                eprintln!("tela-win32-sdk: WGPU device lost, rebuilding once: {detail}");
                self.gpu = None;
    ...
    fn begin_close(&mut self) {
        ...
        if let Err(error) = self.dispatch_text_channel_action(action) {
            eprintln!("tela-win32-sdk: close text channel: {error}");
        }
    }
