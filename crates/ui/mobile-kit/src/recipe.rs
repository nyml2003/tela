//! Mobile 的内置 Headless component recipe。
//!
//! 这个 recipe 把同一 Root/Part 契约投影为更大的触控目标和纵向信息密度。它不根据
//! iOS/Android 名称分支，也不提供外部可替换 recipe trait。
...
use crate::MobileTheme;

/// Mobile recipe 无法投影一个 Root 时的错误。
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum MobileRecipeError {
    /// Root 不满足其 Headless 契约。
    InvalidRoot(HeadlessBuildError),
    /// 该组件仅有 desktop 形态的内置 recipe。
    UnsupportedComponent(&'static str),
}

/// 为一个 Headless Root 构建 mobile 视觉树的内置 recipe。
pub struct MobileRecipe<'a> {
    root: &'a ComponentRoot,
    theme: MobileTheme,
    contents: BTreeMap<SemanticKey, UiNode>,
}

impl<'a> MobileRecipe<'a> {
    /// 用默认移动主题创建 recipe。
    pub fn new(root: &'a ComponentRoot) -> Self {
        Self::themed(root, MobileTheme::default())
    }

    /// 用调用方提供的移动主题创建 recipe。
    pub fn themed(root: &'a ComponentRoot, theme: MobileTheme) -> Self {
        Self {
            root,
            theme,
    ...
    /// 投影 Root 与所有当前部件。
    pub fn into_node(mut self) -> Result<UiNode, MobileRecipeError> {
        self.root
            .validate_complete()
            .map_err(MobileRecipeError::InvalidRoot)?;
        if !self.root.spec().contract().recipes.mobile {
            return Err(MobileRecipeError::UnsupportedComponent(
                self.root.spec().name,
            ));
        }
...
fn decorate_root(root: &ComponentRoot, body: UiNode, theme: MobileTheme) -> UiNode {
    ...
    LayoutContainer::frame(body)
        ...
        .identity(IdentityConcern {
            ...
            semantic_key: Some(root_key(root, "mobile")),
            key_segment: None,
...
fn compose_body(
    ...
    let panel: UiNode = LayoutContainer::column(overlay)
        ...
        .into();
    let align = match root.spec().name {
        "ActionSheet" | "ShareSheet" | "Sheet" | "Popup" => StackAlign::BottomCenter,
        _ => StackAlign::Center,
    };
...
fn default_part_content(root: &ComponentRoot, part: &ComponentPart, theme: MobileTheme) -> UiNode {
...
fn decorate_part(
    ...
    let natural_height = theme.typography.body * theme.typography.line_height + theme.spacing.md;
    let height = if matches!(part.role(), ComponentPartRole::Input)
        && matches!(root.spec().name, "Textarea" | "Mentions")
    {
        theme.min_touch_target * 2.5
    } else if continuous_surface {
        theme.min_touch_target * 3.0
    } else if interactive {
        theme.min_touch_target
    } else {
        natural_height.max(24.0)
    };
...
fn input_spec(root: &ComponentRoot, role: ComponentPartRole) -> Option<TextInputSpec> {
    ...
    let kind = match root.spec().name {
        ...
        "InputNumber" | "NumberKeyboard" => TextInputKind::Number,
...
mod tests {
    ...
    use tela_contract::{
        BaseSize, InputEvent, KeyboardIntent, KeyboardIntentEvent, Point, PointerButtons,
        PointerEvent, PointerId, PointerKind, PointerPhase, SemanticKey, Size, TextMeasureRequest,
        TextMeasurer, TextMetrics, UiAction, UiNode, Viewport,
    };
    ...
    use super::{MobileRecipe, MobileRecipeError, MobileTheme};
    ...
    fn node_for(root: &ComponentRoot) -> UiNode {
        MobileRecipe::new(root)
            .into_node()
            .expect("valid mobile recipe")
    }
    ...
    fn node_with_key<'a>(node: &'a UiNode, key: &SemanticKey) -> Option<&'a UiNode> {
        if node
            .identity
            .as_ref()
    ...
    fn fixed_height(node: &UiNode) -> Option<f32> {
        match node.layout.as_ref()?.height? {
            Size::Raw(BaseSize::Fixed(height)) => Some(height),
            Size::Constrained(_) | Size::Raw(BaseSize::Percent(_) | BaseSize::Auto) => None,
        }
    }
    ...
    #[test]
    fn every_mobile_catalog_recipe_projects_a_valid_tree() {
        for spec in COMPONENT_CATALOG {
            if !spec.contract().recipes.mobile {
                continue;
            }
            let root = spec.root(format!("matrix.mobile.{}", spec.name));
            let node = MobileRecipe::new(&root)
                .into_node()
    ...
    #[test]
    fn desktop_only_roots_are_not_faked_as_mobile_components() {
        let spec = tela_ui_headless::component_spec("Transfer").expect("catalog entry");
        let root = spec.root("desktop.transfer");
        assert_eq!(
            MobileRecipe::new(&root).into_node(),
            Err(MobileRecipeError::UnsupportedComponent("Transfer"))
        );
    }

    #[test]
    fn interactive_parts_use_the_minimum_touch_target_without_bind_id() {
        let root = tela_ui_headless::components::Button::root("mobile.save").part(
            ComponentPartRole::Trigger,
            SemanticKey("mobile.save.trigger".to_owned()),
        );
        let node = MobileRecipe::new(&root).into_node().expect("recipe");
        let tree = UiTree::new(node).expect("valid tree");
        let key = SemanticKey("mobile.save.trigger".to_owned());
        let trigger = tree
            .interact_for_key(&key)
            .expect("explicit trigger interaction");
        assert!(trigger.bind_id.is_none());
    }

    #[test]
    fn every_mobile_root_executes_its_required_matrix_columns() {
        let min_touch_target = MobileTheme::default().min_touch_target;
        for spec in COMPONENT_CATALOG {
            if !spec.contract().recipes.mobile {
                continue;
            }
            ...
            for state in spec.contract().states {
                let baseline =
                    state_context(spec.root(format!("matrix.mobile.{}", spec.name)), *state);
            ...
            match matrix.touch {
                MatrixApplicability::Required => {
                    let interactive_parts: Vec<_> = root
                        .parts()
                        .iter()
                        .filter(|part| {
                            default_tree
                                .interact_for_key(part.key())
                                .is_some_and(|interact| {
                                    interact.clickable
                                        || interact.pointer_capture
                                        || interact.gestures
                                            != tela_contract::GestureConfig::default()
                                })
                        })
                        .collect();
                    assert!(
                        !interactive_parts.is_empty(),
                        "{} requires a touch/pointer path",
                        spec.name
                    );
                    for part in interactive_parts {
                        let node = node_with_key(&default_node, part.key())
                            .expect("interactive part must remain in the recipe tree");
                        assert!(
                            fixed_height(node).is_some_and(|height| height >= min_touch_target),
                            "{} {:?} must meet the mobile minimum touch target",
                            spec.name,
                            part.role()
                        );
                    }
            ...
            if matrix.disabled.is_required() {
                let disabled = spec
                    .root(format!("matrix.mobile.{}.disabled", spec.name))
            ...
            if matrix.loading.is_required() {
                let loading = spec
                    .root(format!("matrix.mobile.{}.loading", spec.name))
    ...
    #[test]
    fn mobile_catalog_state_tree_reference_is_stable() {
        let mut hash = 14_695_981_039_346_656_037_u64;
        for spec in COMPONENT_CATALOG {
            if !spec.contract().recipes.mobile {
                continue;
            }
            let root = spec.root(format!("reference.mobile.{}", spec.name));
            for state in
                std::iter::once(None).chain(spec.contract().states.iter().copied().map(Some))
            {
                let node = match state {
                    Some(state) => node_for(
                        &state_context(
                            spec.root(format!("reference.mobile.{}.{state:?}", spec.name)),
                            state,
                        )
        ...
        assert_eq!(
            hash, 13_177_727_149_878_978_898,
