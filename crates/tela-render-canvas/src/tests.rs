...
fn translates_commands_in_tree_order_with_clip() {
    let frame = UiFrame {
        viewport: Viewport { width: 100.0, height: 50.0 },
        commands: vec![
            DrawCommand {
                geometry: Rect { x: 0.0, y: 0.0, w: 100.0, h: 50.0 },
                clip: None,
                payload: DrawPayload::Rect { fill: Some(Color::WHITE), border: None },
            },
            DrawCommand {
                geometry: Rect { x: 10.0, y: 10.0, w: 40.0, h: 20.0 },
                clip: Some(tela_contract::ClipRect { rect: Rect { x: 0.0, y: 0.0, w: 50.0, h: 30.0 } }),
    ...
    assert_eq!(canvas.calls[0], "fill_rect Rect { x: 0.0, y: 0.0, w: 100.0, h: 50.0 } #FFFFFFFF");
...
fn degrades_by_capabilities() {
    let frame = UiFrame {
        viewport: Viewport { width: 50.0, height: 50.0 },
        commands: vec![DrawCommand {
            geometry: Rect { x: 0.0, y: 0.0, w: 20.0, h: 20.0 },
            clip: None,
            payload: DrawPayload::RoundedRect {
                fill: Some(Color::RED),
                border: None,
        ...
        commands: vec![DrawCommand {
            ...
            payload: DrawPayload::LinearGradient {
                gradient: Gradient {
                    ...
                    stops: vec![
                        tela_contract::ColorStop { position: 0.0, color: Color::RED },
                        tela_contract::ColorStop { position: 1.0, color: Color::BLUE },
                    ],
                },
            },
        }],
    ...
    assert!(canvas.calls[0].starts_with("fill_rect "), "渐变降级为起始色");
}
