...
fn app_bar(title: &str, can_go_back: bool, width: f32) -> UiNode {
    let leading = if can_go_back {
        icon_button(IconName::ArrowBack, "mobile.back", "返回")
    } else {
        icon_badge(IconName::FolderOpen, FOLDER, FOLDER_SURFACE)
    };
    ...
    LayoutContainer::row([
        leading,
        title,
        icon_badge(IconName::More, SECONDARY, MUTED_SURFACE),
    ])
    .layout(LayoutConcern {
        width: Some(Size::fixed(width)),
...
fn search_field(query: &str, focused: bool, width: f32) -> UiNode {
    ...
    let inner: UiNode = LayoutContainer::row([
        icon(IconName::Search, if focused { PRIMARY } else { SECONDARY }),
    ...
    .into();
    let mut field: UiNode = LayoutContainer::frame(inner)
        .layout(LayoutConcern {
            width: Some(Size::fixed((width - CONTENT_INSET * 2.0).max(1.0))),
            height: Some(Size::fixed(SEARCH_FIELD_H)),
            padding: Insets {
                top: 0.0,
                right: 16.0,
                bottom: 0.0,
        ...
        .visual(VisualConcern {
            fill: Some(Fill::Solid(if focused { SURFACE } else { MUTED_SURFACE })),
            border_color: Some(if focused { PRIMARY } else { BORDER }),
            border_radius: BorderRadius::all(8.0),
            ..VisualConcern::default()
        })
        .identity(semantic_identity("mobile.search"))
        .into();
    field.interact = Some(InteractConcern {
        clickable: true,
        focusable: true,
        text_input: true,
        bind_id: Some(BindId("mobile.search".to_owned())),
...
fn browse_list(entries: &[&Entry], width: f32, height: f32, query: &str) -> UiNode {
    let rows: Vec<UiNode> = if entries.is_empty() {
        vec![empty_state(query)]
    } else {
        entries
            .iter()
            .map(|entry| entry_row(entry, width))
            .collect()
    };
...
fn entry_row(entry: &Entry, width: f32) -> UiNode {
    ...
    let content: UiNode = LayoutContainer::row([
        icon_badge(icon_name, icon_color, icon_surface),
        details,
        icon(IconName::ChevronRight, SECONDARY),
    ])
    .layout(LayoutConcern {
        gap: 12.0,
        cross_align: tela_contract::CrossAlign::Center,
    ...
    .into();
    let mut row: UiNode = LayoutContainer::frame(content)
        .layout(LayoutConcern {
            width: Some(Size::fixed(width.max(1.0))),
            height: Some(Size::fixed(ROW_H)),
            padding: Insets {
                top: 8.0,
                right: 12.0,
                bottom: 8.0,
        ...
        .identity(semantic_identity(&format!("mobile.entry.{}", entry.id)))
        .into();
    row.interact = Some(InteractConcern {
        clickable: true,
        focusable: true,
        bind_id: Some(BindId(format!("mobile.entry.{}", entry.id))),
        ..InteractConcern::default()
    });
    row
}

fn preview(entry: &Entry, width: f32, height: f32) -> UiNode {
    let (icon_name, icon_color, icon_surface) = entry_style(entry.kind);
...
fn icon_button(icon_name: IconName, bind_id: &str, _label: &str) -> UiNode {
    let mut button: UiNode = LayoutContainer::frame(icon(icon_name, PRIMARY))
        .layout(LayoutConcern {
        ...
        .identity(semantic_identity(bind_id))
        .into();
    button.interact = Some(InteractConcern {
        clickable: true,
        focusable: true,
        bind_id: Some(BindId(bind_id.to_owned())),
        ..InteractConcern::default()
    });
    button
}

fn icon_badge(icon_name: IconName, color: Color, surface: Color) -> UiNode {
    LayoutContainer::frame(icon(icon_name, color))
        .layout(LayoutConcern {
...
fn icon(name: IconName, color: Color) -> UiNode {
    Icon::new(name).size(24.0).color(color).into_node()
}
