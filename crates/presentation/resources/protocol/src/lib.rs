...
use tela_contract::{TextStyleRef, TextureRef};
...
/// 资源加载端口。
///
/// 适配器内部持有 `TextureRef`/`TextStyleRef` 到 URL、data URI、平台 asset 等来源的映射；
...
pub trait ResourceAdapter {
    ...
    /// 确保指定排版样式对应的字体资源加载已开始；重复调用必须幂等。
    fn request_text_style(&mut self, text_style: &TextStyleRef) -> ResourceState;
