...
use tela_contract::{
    Color, DrawCommand, DrawPayload, FontRef, Rect, TextContent, UiFrame, Viewport,
};
...
pub fn android_main(app: AndroidApp) {
    ...
    let event_loop = match builder.build() {
        ...
        Err(error) => {
            eprintln!("tela-android-sdk: create Android event loop: {error}");
            return;
        }
    };
    ...
    if let Err(error) = event_loop.run_app(&mut host) {
        eprintln!("tela-android-sdk: event loop stopped: {error}");
    }
    clear_bridge();
}
...
impl AndroidHost {
    ...
    fn fail(&mut self, error: String) {
        eprintln!("tela-android-sdk: {error}");
...
impl ApplicationHandler<HostEvent> for AndroidHost {
    ...
    fn memory_warning(&mut self, _event_loop: &ActiveEventLoop) {
        ...
        eprintln!("tela-android-sdk: Android reported low memory");
    }
}
...
fn load_guest(index_url: &str) -> Result<GuestRuntime, String> {
    ...
    eprintln!(
        "tela-android-sdk: bundle archive={}KB download={}ms; initializing guest",
