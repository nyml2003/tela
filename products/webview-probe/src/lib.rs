//! Static browser product assembly for the Tela WebView Probe.
//!
//! The application, session runtime, WebView target, renderer, and resources are linked into one
//! WebAssembly module. JavaScript only drives this concrete session and never imports or
//! instantiates a second guest module.
...
use tela_webview_probe::{WebviewProbeApp, new_webview_probe};
...
#[wasm_bindgen]
pub struct CardDispatchOutcome {
    outcome: AppDispatchOutcome,
}

#[wasm_bindgen]
impl CardDispatchOutcome {
...
impl Default for CardWebSession {
    fn default() -> Self {
        Self::new()
    }
}
...
#[wasm_bindgen]
impl CardWebSession {
    /// Creates a fresh Tela application runtime for the WebView probe page.
    #[wasm_bindgen(constructor)]
    pub fn new() -> Self {
        Self {
            app: new_webview_probe(&RESOURCES),
            pending: None,
    ...
    pub fn initialize(&mut self) -> Result<CardDispatchOutcome, JsValue> {
        self.ensure_open()?;
        if self.initialized {
            return Err(js_error("card session is already initialized"));
        }
    ...
    pub fn dispatch(&mut self, packet: &[u8]) -> Result<CardDispatchOutcome, JsValue> {
        self.ensure_ready()?;
    ...
    pub fn presented(&mut self, raw_token: u64) -> Result<CardDispatchOutcome, JsValue> {
        self.ensure_ready()?;
...
impl CardWebSession {
    fn ensure_open(&self) -> Result<(), JsValue> {
        if self.closed {
            Err(js_error("card session is closed"))
        } else {
            Ok(())
        }
    }
    ...
    fn ensure_ready(&self) -> Result<(), JsValue> {
        ...
        if self.initialized {
            ...
            Err(js_error("card session is not initialized"))
        }
    }
}

fn outcome(outcome: AppDispatchOutcome) -> CardDispatchOutcome {
    CardDispatchOutcome { outcome }
}
...
mod tests {
    ...
    fn static_session_initializes_and_publishes_without_a_guest_module() {
        let mut session = CardWebSession::new();
        assert!(
            session
                .initialize()
                .expect("initialize")
    ...
    #[test]
    fn static_session_delivers_hover_pointer_events() {
        let mut session = CardWebSession::new();
        session.initialize().expect("initialize");
        let publication = session.publish().expect("publication");
        let token = publication.status().frame_token().expect("initial token");
        session.presented(token).expect("initial presented");

        let viewport = tela_target_webview::event_viewport(800.0, 600.0).expect("viewport");
        session.dispatch(&viewport).expect("dispatch viewport");
        let publication = session.publish().expect("viewport publication");
        let token = publication.status().frame_token().expect("viewport token");
        session.presented(token).expect("viewport presented");

        // The card centers in the 800×600 viewport at (400, 300); phase 1 = move, kind 0 = mouse.
        let pointer =
            tela_target_webview::event_pointer(token, 0, 0, 1, 400.0, 300.0, 0, 0, 0.0, 0.0)
                .expect("pointer move");
        let result = session.dispatch(&pointer).expect("dispatch pointer");
        assert!(result.publish_requested());
        let publication = session.publish().expect("hover publication");
        let token = publication.status().frame_token().expect("hover token");
        session.presented(token).expect("hover presented");
    }
}
