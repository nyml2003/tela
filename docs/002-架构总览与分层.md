...
tela-guest-runtime 中性 Wasmtime guest / 严格远程 bundle 校验 / 无窗口 verifier
tela-webview-sdk  浏览器 WebView 胶水（bundle/ABI/WGPU；JS DOM 半边在 web/）
tela-native-sdk-runtime 桌面壳共享的 bundle cache / 生命周期 runtime
tela-win32-sdk    Win32 开发壳（HWND + WGPU）
tela-macos-sdk    macOS 开发壳（AppKit + Metal/WGPU）
tela-android-sdk  Android GameActivity / Vulkan / JNI 输入壳（动态 mobile bundle）
tela-ios-sdk      iPhone UIKit / Metal / Winit 输入壳（静态 mobile app）
tela-demo         desktop 文件管理器 guest
tela-mobile-demo  独立移动文件浏览 guest
...
- ✅ 依赖方向为“宿主 → 适配/分子组件 → 原子组件 → 核心 → 契约”；图标资源支路只向 `tela-core` 的已有原语降级。核心与渲染器的反向依赖、`tela-icon` 与 `tela-ui` 到 renderer/demo 的反向依赖、`tela-webview-sdk -> tela-demo`、`tela-android-sdk -> *demo`，以及 iOS 静态壳错误接入 desktop guest/WASM ABI/bundle/runtime 的依赖均由 `ops check` 拦截。
