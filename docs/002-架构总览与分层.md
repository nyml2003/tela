...
- ✅ 依赖方向为“宿主 → 适配/分子组件 → 原子组件 → 核心 → 契约”；图标资源支路只向 `tela-core` 的已有原语降级。核心与渲染器的反向依赖、`tela-icon` 与 `tela-ui` 到 renderer/demo 的反向依赖、`tela-webview-sdk -> tela-demo` 以及 `tela-android-sdk -> *demo` 的反向应用依赖由 `ops check` 拦截。
