...
> **状态：✅ 已完成。** 本文是 [015-tela-ui 与局部交互状态](015-tela-ui与局部交互状态.md)
> 的第一张实施卡：已验证 crate 边界、组合组件迁移和统一意图出口；自动依赖追踪、Scheduler
> 与复杂局部状态仍留待后续阶段。
...
- 动态集合的身份策略只能封装在集合组件内部：虚拟列表复用稳定业务 ID，非虚拟集合可声明 core 的 `auto-stable-identity`；都不泄漏为调用方必须维护的 tela key。
