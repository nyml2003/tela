...
> **状态：🔧 Win32、macOS 与通用 WebView 开发态已落地。** 本文定义平台 SDK 的最小边界、可验证 WASM 开发包和后续平台的扩展方式；它不承诺生产分发方案。
...
tela-demo 应用源码
  ...
  -> Wasmtime guest（Win32/macOS）或浏览器 WebAssembly guest（WebView）
  -> UiFrame
  -> tela-render-wgpu -> HWND / NSView / HTMLCanvasElement
```

原生开发态每次**进程启动**请求一次包。没有热更新、轮询、跨版本状态迁移、后台预取或运行期二次下载；请求失败时仅回退到本机最后一个仍通过校验的包。窗口先显示原生 loading 页，随后由一次启动 worker 执行请求和 Wasmtime 初始化，UI 线程不会因网络或 guest 编译失去 native event loop。
...
| `tela-native-sdk-runtime` | HTTP bundle loader、最后有效缓存、Wasmtime guest、无窗口 verifier、原生生命周期 | 窗口、WGPU surface、平台输入 |
...
`ops build bundle` 总是将 `tela-demo` 的 `app-wasm` 编译为 **release WASM**，生成：
...
`ops build bundle` 先写 `tela-demo.tela.tmp` 和 `latest.json.tmp`，随后执行 `tela-sdk-verify <tmp>`。验证器检查 archive、以 Wasmtime 实例化 guest、读取首帧并派发一次 viewport；只有通过后才依次替换 archive 与索引。验证失败时运行中的服务仍可提供上一份完整包。
...
Windows 默认请求 `http://127.0.0.1:8000/tela-dev/latest.json`；`--port` 只替换本机端口，`--bundle-index` 允许完整 HTTP(S) 地址，两者互斥。Mac 或局域网开发应使用开发机可达地址和显式 `--bundle-index`。`--verbose` 会输出索引、缓存、网络/缓存来源、下载、compile/init 与 fuel 诊断。`ops serve` 只服务 `dist/`，不会启动浏览器。
...
不预建 Android/iOS 空 crate。出现真实窗口、输入、发布或 bridge 约束时，各自增加薄 SDK，并复用 `tela-app-abi`、`tela-bundle` 与 renderer；不要为了名义统一先创造共享大接口。

## 9. 验收边界

- `ops build bundle` 能从干净的 `dist/` 产出 release guest 包，并在 Wasmtime 首帧/viewport 校验通过后最后发布索引。
