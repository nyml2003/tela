...
> **状态：🟢 目录与依赖迁移已由 [029-迁移记录](029-迁移记录.md) 落地。** 本文保留当时的
> 产品闭包设计与审计基线；`tela-ui-headless`、114 根组件目录及事件边界的后续实现由
> [030-通用组件体系重构方案](030-通用组件体系重构方案.md) 记录并约束。
> 第 3 节的“现状”是迁移前审计快照，不应再被当成当前 crate 图；各平台的实际构建和图形验收仍以
> 对应平台文档为准。
>
...
package "UI System" {
  component "ui-dsl\nContext / Signal / ActionFrame" as dsl
  component "ui-headless\nRoot-Part / EventRegistry" as headless
...
desktop_app --> dsl
mobile_app --> dsl
desktop_app --> headless
mobile_app --> headless
desktop_kit --> headless
mobile_kit --> headless
...
### 4.2 UI System：headless、foundation、desktop kit、mobile kit
...
desktop_view --> dsl
mobile_view --> dsl
desktop_view --> headless
mobile_view --> headless
desktop_kit --> headless
mobile_kit --> headless
desktop_kit --> foundation
mobile_kit --> foundation
...
headless --> kernel
foundation --> kernel
normalized --> kernel : normalized input / viewport / insets
...
| 子域 | 责任 | 不应拥有 |
|---|---|---|
| `ui-dsl` | Application Context、Signal、显式失效、帧级 ActionFrame 与视图组合；只将本帧 Kernel 动作映射为应用 Action | Headless Root/Part 契约、视觉 token、kit recipe、Renderer、Target API 或业务 Store |
| `ui-headless` | Root/Part 契约、稳定部件路径和帧级 EventRegistry；只把 Kernel 动作转为类型化组件事件 | Signal、显式失效、视觉 token、kit recipe、Renderer、Target API、业务 Store 或 `BindId` 命令编码 |
| `ui-foundation` | 两端完全一致的语义原语、可访问交互意图、间距/颜色/文字/图标的抽象 token，以及确实相同的低层受控控件 | 平台样式分支、Material 字体实现、业务页面、桌面或移动专属导航 |
| `desktop-ui-kit` | 键鼠优先、信息密度、form / table / toolbar / segmented / pagination、desktop 对话框及带动作的状态与空反馈 | Win32/AppKit API、mobile safe-area 查询、mobile 业务视图 |
| `mobile-ui-kit` | 安全区内的页面 scaffold、单列 cell / group、顶部导航、44pt/48dp 触控策略、tabs、带显式取消路径的 action sheet、搜索和空态 | UIKit/Android 触控转换、IME 实现、手势识别器、desktop 业务视图 |
