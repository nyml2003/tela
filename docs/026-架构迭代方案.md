...
package "UI System" {
  component "ui-headless\nSignal / Root-Part / EventRegistry" as headless
...
| 子域 | 责任 | 不应拥有 |
|---|---|---|
| `ui-headless` | Signal、显式失效、Root/Part 契约、稳定部件路径和帧级 EventRegistry；只把 Kernel 动作转为类型化组件事件 | 视觉 token、kit recipe、Renderer、Target API、业务 Store 或 `BindId` 命令编码 |
