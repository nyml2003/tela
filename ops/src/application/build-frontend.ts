// 应用层：build frontend 用例——WebView 产品源码 esbuild 构建到 dist/assets/tela-web/。
...
/** 构建浏览器产品页面（build.mjs：页面模板 + bundle → dist/）。 */
export async function runBuildFrontend(
  ...
  const t0 = Date.now();
  reporter.section('构建 WebView 产品前端（products/webview → dist/assets/tela-web）');
  // 直接 node build.mjs（绕开 pnpm 11 的 verify-deps-before-run 前置检查；
  // 依赖由 pnpm install 管理，构建本身只需 node + node_modules）。
  const result = await process.run('node', ['build.mjs'], { cwd: workspace.webviewProductDir });
