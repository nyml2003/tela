...
import { runBuildAgentDemo } from './build-agent-demo.ts';
...
test('静态 Agent 产品经 wasm-bindgen 发布为单一模块', async () => {
  const calls: string[] = [];
  ...
  assert.deepEqual(builds, [['tela-product-agent-demo', 'release']]);
  assert.deepEqual(calls, [
    'dir:/repo/dist',
    'wasm-bindgen:--target web --out-dir /repo/dist --out-name tela_agent_demo /repo/target/wasm32-unknown-unknown/release/tela_product_agent_demo.wasm',
  ]);
});
